...
namespace mcts {
	...
	float MCTS::evaluate(const DerivationTree& derivationTree) {
		...
		cv::Mat tmp;
		cv::reduce((distMap - targetDistMap).mul(distMap - targetDistMap), tmp, 0, CV_REDUCE_SUM);
		cv::reduce(tmp, tmp, 1, CV_REDUCE_SUM);

		return expf(-tmp.at<float>(0, 0) * 0.001f);
	}

	void MCTS::generateGeometry(RenderManager* renderManager, glm::mat4& modelMat, float length, float width, const boost::shared_ptr<Nonterminal>& node, std::vector<Vertex>& vertices) {
		if (node->name == "F") {
			glm::mat4 mat = glm::translate(modelMat, glm::vec3(0, length * 0.5, 0));
			glutils::drawQuad(width, length, glm::vec4(0, 0, 0, 1), mat, vertices);
			modelMat = glm::translate(modelMat, glm::vec3(0, length, 0));
		}
		else if (node->name == "/") {
			modelMat = glm::rotate(modelMat, node->angle / 180.0f * M_PI, glm::vec3(0, 0, 1));
		}
		else if (node->name == "\\") {
			modelMat = glm::rotate(modelMat, node->angle / 180.0f * M_PI, glm::vec3(0, 0, 1));
		}
		...
		for (int i = 0; i < node->children.size(); ++i) {
			generateGeometry(renderManager, modelMat, length, width, node->children[i], vertices);
		}
	}
